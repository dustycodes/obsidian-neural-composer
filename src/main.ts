...
onunload() {
  ...
  public generateEnvConfig(): string {
    ...
    try {
        ...
        /** LightRAG `CHUNK_SIZE` is tokenizer tokens (`chunk_token_size`), not chars. Typical Ollama BERT embeddings (e.g. mxbai) use ~512 context. */
        const OLLAMA_EMBED_CHUNK_SAFE = 512;
        let chunkSizeOut = this.settings.lightRagChunkSize;
        let chunkOverlapOut = this.settings.lightRagChunkOverlap;
        let chunkOverlapNote = '';
        if (embedModelObj && embedProvider) {
            const preEmbedBinding =
                this.lightRagBindingFromNcModelProviderType(embedModelObj.providerType) ??
                this.lightRagBindingForProvider(embedProvider);
            const preResolvedEmbedBinding =
                preEmbedBinding !== null ? preEmbedBinding : 'openai';
            if (
                preResolvedEmbedBinding === 'ollama' &&
                chunkSizeOut > OLLAMA_EMBED_CHUNK_SAFE
            ) {
                chunkSizeOut = OLLAMA_EMBED_CHUNK_SAFE;
                chunkOverlapNote =
                    `\n# Note: CHUNK_SIZE capped at ${OLLAMA_EMBED_CHUNK_SAFE} tokens for Ollama BERT embeddings; raise via custom env if your model supports more.\n`;
                if (chunkOverlapOut >= chunkSizeOut) {
                    chunkOverlapOut = Math.max(
                        32,
                        Math.floor(chunkSizeOut * 0.1),
                    );
                }
            }
        }
        ...
        envContent += `CHUNK_SIZE=${chunkSizeOut}\n`;
        envContent += `CHUNK_OVERLAP_SIZE=${chunkOverlapOut}\n\n`;
...
// Embeddings (Smart Mapping)
        if (embedModelObj && embedProvider) {
            ...
            // Used by LightRAG for summaries / decorator max_token caps; harmless for embeddings.
            const resolvedEmbedBinding =
                embedBinding !== null ? embedBinding : 'openai';
            if (resolvedEmbedBinding === 'ollama') {
                envContent += `EMBEDDING_TOKEN_LIMIT=${OLLAMA_EMBED_CHUNK_SAFE}\n`;
            }
        }
